...
- `camera.py`: 운영체제별 카메라 탐색과 OpenCV 캡처 생성을 담당한다. 연결된
  장치 목록도 여기서 만들고, 조회 결과를 캐시해 창마다 다시 열어 보지 않는다.
- `camera_select.py`: 카메라 선택 위젯. 조회는 워커 스레드에서 하고 결과 반영만
  메인 스레드로 돌려준다.
